...
const stream2 = kafkaStreams.getKStream("topic2");
...
{
    //stream2.forEach(message => console.log(`Stream2 ${message}`));
    surfacestream(stream2,'stream2')
}
{
    let mergestream = stream2.merge(stream1)
    surfacestream(mergestream,'merge')
}


function surfacestream(stream,streamName){
    stream.forEach(message => console.log(`Stream[${streamName}]\t${message}`));
    stream.start().then(() => {
